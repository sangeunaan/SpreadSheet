...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name); // add 'name' to 'visited'
            foreach (String n in GetDirectDependents(name)) // for each dependent of name
            {
                if (n.Equals(start))    // if it is the same as 'start'
                {
                    throw new CircularException(); // then throw a circular reference
                }
                else if (!visited.Contains(n))  // otherwise if it has not yet been visited
                {
                    Visit(start, n, visited, changed);  // check all of its dependees for circular references with 'start'
                }
            }
            changed.AddFirst(name); // once recalculated, add 'name' to 'changed'
        }
